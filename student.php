...
<body>
    <?php
        ...
        // validation
        if (empty($COURSES) or empty($rn) or preg_match("/[a-z]/i",$rn)) {
            if(empty($COURSES))
        ...
        $name_sql=mysqli_query($conn,"SELECT `name` FROM `students` WHERE `rno`='$rn' and `class_name`='$COURSES'");
        while($row = mysqli_fetch_assoc($name_sql))
        {
        $name = $row['name'];
        }

        $result_sql=mysqli_query($conn,"SELECT `p1`, `p2`, `p3`, `p4`, `p5`, `marks`, `percentage` FROM `result` WHERE `rno`='$rn' and `courses`='$COURSES'");
    ...
    <div class="container">
        <!-- <div class="details">
            ...
            <span>Class:</span> <?php echo $COURSES; ?> <br>
